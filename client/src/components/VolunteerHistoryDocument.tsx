...
const VolunteerHistoryDocument = () => {
    ...
    useEffect(() => {
        axios.get(`${base_url}/api/volunteers/mongo`)
             .then((response) => setVolunteers(response.data))
             .catch((error) => console.error("Error fetching volunteers:", error));
        axios.get(`${base_url}/api/history/mongo`)
             .then((response) => {
    ...
    const createTableHeader = () => {
        ...
        return (
            <View style={tableRowStyle} fixed>
                ...
                <View style={skillsHeaderCellStyle}>
                ...
                <View style={tableStyle}>
                    {createTableHeader()}
                    {
                        volunteers?.map(volunteer => (
                            ...
                            <View style={cellStyle}>
                                <Text style={idTextStyle}>{volunteer._id}</Text>
                            </View>
                            <View style={cellStyle}>
                                <Text style={idTextStyle}>{volunteer.userId}</Text>
                            </View>
                            <View style={cellStyle}>
                                <Text style={textStyle}>{volunteer.preferences || "None"}</Text>
                            </View>
                            <View style={skillsCellStyle}>
                                <Text style={textStyle}>{volunteer.skills.join(", ")}</Text>
                            </View>
                            <View style={cellStyle}>
                                <Text style={textStyle}>{getParticipatedEvents(volunteer._id)}</Text>
                            </View>
                            <View style={cellStyle}>
                                <Text style={textStyle}>{getNoShowEvents(volunteer._id)}</Text>
                            </View>
                            <View style={cellStyle}>
                                <Text style={textStyle}>{getScheduledEvents(volunteer._id)}</Text>
                            </View>
                        </View>
                        ))
                    }
...
const skillsHeaderCellStyle: Style = { flex: 1.5, backgroundColor: "#f0f0f0", padding: 4, borderWidth: 1, borderColor: "#000" }; // Wider Skills column
const cellStyle: Style = { flex: 1, padding: 4, borderWidth: 1, borderColor: "#000" };
const skillsCellStyle: Style = { flex: 1.5, padding: 4, borderWidth: 1, borderColor: "#000" }; // Wider Skills column
const idTextStyle: Style = { 
    fontSize: 7, 
    textAlign: "center", 
    overflow: "hidden", 
    padding: 2 
